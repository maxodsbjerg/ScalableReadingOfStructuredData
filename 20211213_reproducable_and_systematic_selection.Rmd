...
As we have now captured  the top 20 most liked tweets, we can now create a new dataset called *sesamestreet_data_favorite_count_over_18*. 
...
To create a quick overview of our new dataset, we use the `select`-function from the dplyr-package to isolate the variables we wish to inspect. In this case, we wish to isolate the columns favorite_count, screen_name, verified and text.

We then arrange them after their favorite_count value by using the `arrange`-function. 
...
To export our new dataset out of our R environment and save it as a JSON file, we use the `toJSON`-function from the jsonlite-package. 
...
We now wish to see  the top 20 most liked tweets by the non-verified accounts.

To do this, we follow the same workflow as before, but in our first code chunk, we include an extra `filter`-function from the dplyr-package which retains all rows with the value FALSE in the verified column, thereby removing all tweets from our data which have been produced by verified accounts. 
...
We observe in the returned data.frame that 809 of the total 2413 observations are not retweets AND are from non-verified accounts. 

Looking again at the favorite_count column, we observe that the top 20 most commented tweets by non-verified accounts all have a count that is above 15. This time, 2 tweets share the 20th and 21th place. We therefore get the top 21 most liked tweets for this analysis.

We can now filter tweets that have been liked more than 15 times, and arrange them from the most commented to the least, and create a new dataset in our Global Environment called *sesamestreet_data_favorite_count_over_15_non_verified*.
